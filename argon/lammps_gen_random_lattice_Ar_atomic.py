...
def lammps_gen_random_lattice_Ar_atomic(**kwargs):
    ...
    if(gen_file):
        ...
        file.write("#   Random lattice with " + str(num_atoms) + " argon atoms, at a density of " + str(density) + " g/cm^3\n")
        ...
        # Create atom arrays
        atom_pos_x = [-1.0 for x in range(num_atoms+1)]
        atom_pos_y = [-1.0 for x in range(num_atoms+1)]
        atom_pos_z = [-1.0 for x in range(num_atoms+1)]
...
if __name__ == '__main__':
    ...
    if( len(sys.argv) > 1 ):
        ...
        while True:
            try:
                if sys.version_info[0] < 3:
                    temperature = float(raw_input('Enter the required temperature [K] : '))
                else:
                    temperature = float(input('Enter the required temperature [K] : '))
                if (temperature > 0):
                    break
                else:
    ...
    lammps_gen_random_lattice_Ar_atomic(verbose=True,num_atoms=num_atoms,temperature=temperature)#,use_min_sep=False)
