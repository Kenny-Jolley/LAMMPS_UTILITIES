...
def lammps_convert_output_to_atomic_data_file(filename, **kwargs):
    ...
    atom_x_pos = [0 for x in xrange(atoms+1)]
    atom_y_pos = [0 for x in xrange(atoms+1)]
    atom_z_pos = [0 for x in xrange(atoms+1)]
    atom_type  = [0 for x in xrange(atoms+1)]
    id_counter = 1
    atom_type_specie  = [0 for x in xrange(114)]
...
if __name__ == '__main__':
    ...
    if("*" not in filename):
        print('no star')
        # call the function
        lammps_convert_output_to_atomic_data_file(filename, verbose=verbose)
    else:
        ...
        for file in file_list:
            if( filename_prefix == file[:len(filename_prefix)]):
                if( filename_sufix == file[-len(filename_sufix):]):
                    print(file)
                    # call the function
                    lammps_convert_output_to_atomic_data_file(file, verbose=verbose)
