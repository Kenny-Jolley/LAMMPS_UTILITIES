                         ...
                         reax_list **lists, output_controls * /*out_control*/ )
{
  ...
  // ZBL constants for C-C interaction
  // some params set in the header file
  double zbl_c[4], zbl_d[4];
  double zblpotl_cc[4], zblpotl_cd[4];
  double zblterm;
  int nn;
  double RLIJ_POW2, RLIJ_POW3, RLIJ_POW4, RLIJ_POW5;
  ...
  for( i = 0; i < 4; ++i ) {
    zblpotl_cc[i] = zbl_c[i] * ZBL_CCONST_CC;
    zblpotl_cd[i] = -zbl_d[i] / ZBL_A_CC;
    //printf("zbl %lf \n",zblpotl_cc[i]);
    //printf("zbl %lf \n",zblpotl_cd[i]);
  }
...
//      printf("num_atom_types = %d\n", system->reax_param.num_atom_types);
//      printf("system->my_atoms[i].type = %d\n", system->my_atoms[i].type);
//      printf("system->my_atoms[j].type = %d\n", system->my_atoms[j].type);
//      printf("r_ij = %lf\n", r_ij);
      ...
      if(system->reax_param.gp.vdw_type==1 || system->reax_param.gp.vdw_type==3)
        { // shielding
          ...
          if(r_ij < AC_C_){
              //printf("zbl: \n");
            for( nn = 0; nn < 4; ++nn ) {
              // POT CALCULATES PHI TERM, ZBL_FOR_ZBLPOTL_CONC CONTAINS GAMMA
              zblterm = zblpotl_cc[nn] * exp( zblpotl_cd[nn] * r_ij );
              // energy term: ADDS THE 4 EXP TERMS IN PHI
              e_vdW = e_vdW + zblterm;
              // force term: DIFF PHI W.R.T.r
              CEvd = CEvd + zblpotl_cd[nn] * zblterm;
            }
            e_vdW = e_vdW / r_ij;
            ...
            CEvd = dTap * e_vdW - Tap * twbp->D * (twbp->alpha / twbp->r_vdW) * (exp1 - exp2) * dfn13;
          }
             //printf("energy term: %lf \n", e_vdW);
        }
      else{ // no shielding
        //printf("no shielding \n");
