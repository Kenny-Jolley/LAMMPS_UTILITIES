...
def lammps_plot_energy_vs_time(**kwargs):
    ...
    if(verbose):
        print("  +-------------------------------------------+")
        print("  | This script reads a lammps log file and   |")
        ...
        print(">  Reading Lammps log file: " + str(filename))
        ...
        while 1:
            ...
            if (header_cols_found == 2):
                ...
                while 1:
                    ...
                    if(fileline[0] == "L"): break
                        
                    # split line into a list
                    fileline = fileline.split()
                        
                    # exit if len < temp or time col
                    if( (len(fileline)< time_col) + (len(fileline)< temp_col) ): break
                ...
                # now we set header cols to zero again, so we search for additional blocks
                header_cols_found = 0
    ...
    if( data_blocks > 1):
        for i in range(0,data_blocks-1):
            ax.plot(data_time[data_block_start_ids[i]:data_block_start_ids[i+1]],
                    ...
                    color=color_list[i], linewidth=1.5, linestyle="-",label=mylabel)

        ax.plot(data_time[data_block_start_ids[data_blocks-1]:],
                data_temp[data_block_start_ids[data_blocks-1]:],
                color=color_list[data_blocks-1], linewidth=1.5, linestyle="-",label=mylabel)
...
if __name__ == '__main__':
    ...
    if( len(sys.argv) > 1 ):
        ...
        lammps_plot_energy_vs_time(verbose=verbose, overwrite=overwrite, filename=filename)
    
    else:
        # call the function safely (use default filename)
        lammps_plot_energy_vs_time(verbose=verbose, overwrite=overwrite)
