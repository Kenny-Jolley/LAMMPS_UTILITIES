...
# This function generates a random lattice of Si, B and O atoms (SiO2 and B2O3 formula units).
...
def lammps_gen_random_SiO2_B2O3_charge(**kwargs):
    # Default keyword args
    verbose = kwargs.get('verbose', False)
    forced = kwargs.get('forced', True)
    num_SiO2 = kwargs.get('num_SiO2', 1000)
    num_B2O3 = kwargs.get('num_B2O3', 1000)
    si_charge = kwargs.get('si_charge', 4)
    o_charge = kwargs.get('o_charge', -2)
    b_charge = kwargs.get('b_charge', 3)
    ...
    total_mass = (num_SiO2 * (si_mass + 2.0 * o_mass) * amu) + (num_B2O3 * (b_mass * 2.0 + 3.0 * o_mass) * amu)

    total_atoms = (3 * num_SiO2) + (5 * num_B2O3)
    ...
    if verbose:
        ...
        print("  |         Random SiO2,B2O3 lattice         |")
        print("  |                                          |")
        print("  |               Kenny Jolley               |")
        print("  |                 Nov 2020                 |")
        print("  +------------------------------------------+")
        ...
        print("  Silicon atoms      : " + str(num_SiO2))
        print("  Boron atoms        : " + str(2*num_B2O3))
        print("  Oxygen atoms       : " + str(2 * num_SiO2 + 3 * num_B2O3))
    ...
    if not forced:
        if os.path.isfile(filename):
            ...
            print("> lammps_gen_random_SiO2_B2O3_charge function wants to overwrite this file")
    ...
    # Write header info
    file.write("Lammps data file generated by lammps_gen_random_SiO2_B2O3_charge\n")
    file.write("#  Random lattice with " +
               str(num_SiO2) + " Si atoms (charge = " + str(si_charge) + ") and " +
               str(2*num_B2O3) + " B atoms (charge = " + str(b_charge) + ") and " +
               str(2 * num_SiO2 + 3 * num_B2O3) + " O atoms (charge = " + str(o_charge) + "), at a density of " +
               str(density) + " kg/m^3\n")
    file.write(str(total_atoms) + " atoms\n\n")
    file.write("3 atom types # O Si B\n\n")
    ...
    for i in range(num_SiO2 + 1, num_SiO2 + 1 + 2*num_B2O3):
        ...
        file.write(str(i) + "  3  " +
                   ...
                   str(atom_pos_z[i]) + "\n")
    # O atoms
    for i in range(num_SiO2 + 1 + 2*num_B2O3, total_atoms + 1):
        # Output to file
        file.write(str(i) + "  1  " +
                   str(o_charge) + "  " +
                   str(atom_pos_x[i]) + "  " +
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        # 6 params = number of SiO2 formula units, Si charge, O charge and density
        if len(sys.argv) == 7:
            _num_SiO2 = int(sys.argv[1])
            _num_B2O3 = int(sys.argv[2])
            _o_charge = float(sys.argv[3])
            _si_charge = float(sys.argv[4])
            _b_charge = float(sys.argv[5])
            _density = float(sys.argv[6])

        else:
            print(">>> ERROR  <<<")
            print("  User must pass 4 command-line arguments")
            print("   6 params = number of SiO2 and B2O3 formula units (integer),"
                  " O charge Si charge, B charge and density  (float) ")
            print("    examples")
            print("   lammps_gen_random_SiO2_B2O3_charge.py 700 300 -2 4 3 2100")
            sys.exit()
    else:
    ...
    lammps_gen_random_SiO2_B2O3_charge(verbose=True,
                                       num_SiO2=_num_SiO2,
                                       num_B2O3=_num_B2O3,
                                       b_charge=_b_charge,
