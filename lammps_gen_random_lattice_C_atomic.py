...
# This function generates a random lattice of carbon atoms.
...
# density = 2.39     , the density of the system [g/cm^3)
# num_atoms = 1000   , the number of carbon atoms in the lattice
...
def lammps_gen_random_lattice_C_atomic(**kwargs):
    # Default keyword args
    verbose = kwargs.get('verbose', False)
    forced = kwargs.get('forced', True)
    density = kwargs.get('density', 2.39)
    ...
    carbon_mass = 12.011
    total_mass = carbon_mass * num_atoms * amu
    
    # System volume
    volume_m3 = total_mass/(1000.0 * density)
    ...
    if(verbose):
        ...
        print("  +------------------------------------------+")
        print("   ")
        print("  Generating a random distribution of Carbon atoms")
        print("  Carbon atoms: " + str(num_atoms))
    ...
    if(not forced):
        if(os.path.isfile(filename)):
            ...
            print("> lammps_gen_random_lattice_C_atomic function wants to overwrite this file")
    ...
    if(gen_file):
        ...
        # Write header info
        file.write("#   Lammps data file generated by lammps_gen_random_lattice_C_atomic\n")
        file.write("#   Random lattice with " + str(num_atoms) + " carbon atoms, at a density of " + str(density) + " g/cm^3\n")
        ...
        file.write("Masses\n\n")
        file.write("1 " + str(carbon_mass) + "\n\n")
        file.write("Atoms # atomic\n\n")
    
        # Create atom arrays
        atom_pos_x = [-1.0 for x in xrange(num_atoms+1)]
        atom_pos_y = [-1.0 for x in xrange(num_atoms+1)]
        atom_pos_z = [-1.0 for x in xrange(num_atoms+1)]
        ...
        for i in range(1, num_atoms+1):
            ...
            # Output to file
            file.write(str(i) + "  1  " + str(atom_pos_x[i]) + "  " + str(atom_pos_y[i]) + "  " + str(atom_pos_z[i]) + "\n")
...
if __name__ == '__main__':
    ...
    if( len(sys.argv) > 1 ):
        ...
        if( len(sys.argv) == 3):
            ...
            print("   2 params = num_atoms (integer), density  (float) ")
            print("    examples")
            print("   lammps_gen_random_lattice_C_atomic.py 1000 2.3")
            print("   lammps_gen_random_lattice_C_atomic.py 5000 2.5")
            print("   lammps_gen_random_lattice_C_atomic.py 9000 2.339")
        ...
        while True:
            try:
                if sys.version_info[0] < 3:
                    num_atoms = int(raw_input('Enter number of atoms required : '))
                else:
                    num_atoms = int(input('Enter number of atoms required : '))
                if (num_atoms > 0):
                    break
                else:
        ...
        while True:
            try:
                if sys.version_info[0] < 3:
                    density = float(raw_input('Enter the required density [g/cm^3] : '))
                else:
                    density = float(input('Enter the required density [g/cm^3] : '))
                if (density > 0):
                    break
                else:
                    print("Oops!  Density must be greater than 0.  Try again...")
            except ValueError:
                print("Oops!  That was not a valid number.  Try again...")


    # call the lattice generator function
    lammps_gen_random_lattice_C_atomic(verbose=True,num_atoms=num_atoms,density=density)#,use_min_sep=False)
