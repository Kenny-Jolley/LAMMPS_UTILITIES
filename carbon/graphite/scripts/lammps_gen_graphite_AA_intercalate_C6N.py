...
# Function create
def lammps_gen_graphite_aa_intercalate_mc6(**kwargs):
    # Default keyword args
    verbose = kwargs.get('verbose', False)
    forced = kwargs.get('forced', False)
    intercalant = kwargs.get('intercalant', 'Li')
    # Default constants
    ...
    # cells in y dir should be divisible by three for periodicity
    if cells[1] % 3 != 0:
        print(f'unit cells in y direction must be divisible by three')
        print('>>>ERROR<<<')
        return
    ...
    if verbose:
        ...
        print("  |       with intercalated element MC6      |")
        print("  |         y dimension divisible by 3       |")
        print("  |               Kenny Jolley               |")
        print("  |                April 2023                |")
        print("  +------------------------------------------+")
        print("   ")
    ...
    if not forced:
        if os.path.isfile(filename):
            ...
            print("> lammps_gen_graphite_AA_intercalate_C6N function wants to overwrite this file")
    ...
    if gen_file:
        ...
        # Write header info
        file.write("Lammps data file generated by lammps_gen_graphite_AA_intercalate_C6N\n")
        file.write(f"# Graphite {cells[0]}x{cells[1]}x{cells[2]} Unit cells, " +
                   f"MC6 intercalation with {intercalant} in AA stacking. " +
        ...
        for x in range(0, cells[0]):
            ...
            for y in range(0, cells[1]):
                ...
                for z in range(0, cells[2]):
                    ...
                    # for third y row
                    if y % 3 == 0:
                        file.write(f'{count}  2  {x_shift + a_const * math.sqrt(3) * 5.0 / 6.0:.12f} ' +
                                   f'{y_shift + a_const / 2.0:.12f} {z_shift:.12f}\n')
                        count += 1
                    elif y % 3 == 1:
                        file.write(f'{count}  2  {x_shift + a_const * math.sqrt(3) / 3.0:.12f} ' +
                                   f'{y_shift + a_const:.12f} {z_shift:.12f}\n')
                        count += 1
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        # 4 params = cell size, user chosen element
            ...
            print("    examples")
            print("   lammps_gen_graphite_AA_intercalate_C6N.py 1 3 1")
            print("   lammps_gen_graphite_AA_intercalate_C6N.py 3 3 3 Li")
            print("   lammps_gen_graphite_AA_intercalate_C6N.py 3 6 2 Na")
            sys.exit()
        ...
        while True:
            try:
                cell_size = int(input('Enter number of cells in x dir : '))
                if cell_size > 0:
                    break
                else:
        ...
        while True:
            try:
                cell_size = int(input('Enter number of cells in y dir (must be divisible by 3): '))
                if cell_size > 0:
                    if cell_size % 3 == 0:
                        break
                    else:
                        print(f'You entered {cell_size} which is not divisible by 3. Try again...')
                else:
    ...
    lammps_gen_graphite_aa_intercalate_mc6(verbose=True,
                                           forced=True,
                                           cells=cells_user,
