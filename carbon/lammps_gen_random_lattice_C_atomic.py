...
def lammps_gen_random_lattice_c_atomic(**kwargs):
    ...
    if gen_file:
        ...
        file.write("#   Random lattice with " + str(num_atoms) +
                   " carbon atoms, at a density of " + str(density) + " g/cm^3\n")
            ...
            while True:
                ...
                if use_min_sep:
                    ...
                    for j in range(0, i):
                        ...
                        # if within x dist, check other coordinates
                        if dxm < min_sep:
                            ...
                            dym = min(dy1, dy2)
                            dym = min(dym, dy3)
                            # if within y dist, check other coordinates
                            if dym < min_sep:
                                ...
                                # if within z dist, check other coordinates
                                if dzm < min_sep:
                                    # full calc
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        if len(sys.argv) == 3:
            num_atoms_user = int(sys.argv[1])
            density_user = float(sys.argv[2])
        else:
            print(">>> ERROR  <<<")
        ...
        while True:
            try:
                num_atoms_user = int(input('Enter number of atoms required : '))
                if num_atoms_user > 0:
                    break
                else:
        ...
        while True:
            try:
                density_user = float(input('Enter the required density [g/cm^3] : '))
                if density_user > 0:
                    break
                else:
    ...
    lammps_gen_random_lattice_c_atomic(verbose=True,
                                       num_atoms=num_atoms_user,
                                       density=density_user)  # ,use_min_sep=False)
